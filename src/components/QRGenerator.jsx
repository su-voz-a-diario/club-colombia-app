  ...
  return (
    <div className="flex flex-col items-center justify-center p-6 bg-[#0e121e] border border-slate-800 rounded-3xl w-full max-w-[300px] relative overflow-hidden shadow-2xl font-sans">
      ...
      <div className="relative p-4 bg-white rounded-2xl z-10 shadow-lg">
        ...
        {/* Láser de verificación en rojo/naranja */}
        {(status === "pending_validation" || status === "on_hold") && (
          <div className={`absolute left-0 right-0 h-0.5 shadow-[0_0_6px_currentColor] animate-pulse top-1/2 -translate-y-1/2 pointer-events-none ${
            status === "pending_validation" ? "bg-red-500 text-red-500" : "bg-amber-500 text-amber-500"
          }`} />
        )}
      </div>
      ...
      <div className="text-center mt-5 z-10 w-full">
        ...
        {status === "pending_validation" && (
          <p className="text-[10px] text-red-500 font-semibold mt-3 animate-pulse bg-red-500/10 py-1.5 px-2.5 rounded-lg border border-red-500/20 leading-tight">
            DEPOSITADO - POR CONFIRMAR
          </p>
        )}
