...
export default function Home() {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-[#07090e] selection:bg-brand-green selection:text-white">
      ...
      <section className="px-6 py-12 border-t border-slate-900 bg-[#07090e] relative overflow-hidden">
        {/* Glow de fondo */}
        <div className="absolute bottom-0 right-1/4 w-[300px] h-[300px] rounded-full bg-emerald-500/5 filter blur-[100px] pointer-events-none" />
        
        <div className="max-w-4xl mx-auto bg-[#0e121e]/80 border border-slate-800 rounded-3xl p-6 sm:p-8 space-y-6 relative z-10">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-800 pb-5">
            <div>
              <span className="text-[9px] font-mono text-amber-500 font-black uppercase tracking-widest block">
                Fase de Despliegue & Arquitectura
              </span>
              <h2 className="font-display font-black text-xl sm:text-2xl text-slate-100 uppercase tracking-wide mt-1">
                Hoja de Ruta: Servidor & Base de Datos Real
              </h2>
            </div>
            <div className="bg-slate-900 border border-slate-800 px-3 py-1.5 rounded-xl text-center md:text-right shrink-0">
              <span className="text-[8px] text-slate-500 block uppercase font-bold">ESTADO DE PROPUESTA</span>
              <span className="text-[10px] text-amber-500 font-bold uppercase tracking-wider flex items-center gap-1 mt-0.5 justify-center md:justify-end">
          ...
          <p className="text-xs text-slate-400 leading-relaxed font-sans max-w-3xl">
            La aplicación actual funciona como una <strong>Propuesta Interactiva (Demo)</strong> con datos simulados guardados temporalmente en el navegador. Para convertirla en una herramienta funcional y segura que el club pueda usar en su día a día, se requiere realizar el montaje de la infraestructura en internet:
          </p>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-5 pt-2">
            {/* Item 1 */}
            <div className="bg-[#07090e] border border-slate-900 p-4 rounded-2xl space-y-2">
              <div className="flex items-center gap-2 text-[#10b981]">
                <Check className="w-4.5 h-4.5" />
                <h4 className="font-display font-bold text-xs uppercase tracking-wider">Base de Datos Privada en la Nube</h4>
              </div>
              <p className="text-[11px] text-slate-400 leading-relaxed">
                Alojamiento seguro (PostgreSQL con cifrado SSL) para guardar los expedientes de los alumnos, bitácoras de asistencia, reportes técnicos y estados de cuenta en un servidor en la nube permanente, garantizando que la información nunca se pierda y sea confidencial.
              </p>
            </div>
            ...
            <div className="bg-[#07090e] border border-slate-900 p-4 rounded-2xl space-y-2">
              <div className="flex items-center gap-2 text-[#10b981]">
                <Check className="w-4.5 h-4.5" />
                <h4 className="font-display font-bold text-xs uppercase tracking-wider">Costos Fijos de Infraestructura</h4>
              </div>
              <p className="text-[11px] text-slate-400 leading-relaxed">
                El montaje real en internet genera costos operativos recurrentes de proveedores de nube (ej. hosting del servidor, base de datos privada, certificado de seguridad HTTPS para transacciones seguras y el registro del dominio personalizado del club).
              </p>
            </div>
            ...
            <div className="bg-[#07090e] border border-slate-900 p-4 rounded-2xl space-y-2">
              <div className="flex items-center gap-2 text-[#10b981]">
                <Check className="w-4.5 h-4.5" />
                <h4 className="font-display font-bold text-xs uppercase tracking-wider">Independencia en los Pagos</h4>
              </div>
              <p className="text-[11px] text-slate-400 leading-relaxed">
                Para evitar comisiones de intermediarios o configuraciones bancarias complejas para el club, el sistema utiliza un método de reporte directo: los padres transfieren o pagan en efectivo, y el administrador valida manualmente el recaudo para rehabilitar el acceso QR.
              </p>
            </div>
            ...
            <div className="bg-[#07090e] border border-slate-900 p-4 rounded-2xl space-y-2">
              <div className="flex items-center gap-2 text-[#10b981]">
                <Check className="w-4.5 h-4.5" />
                <h4 className="font-display font-bold text-xs uppercase tracking-wider">Control Total del Administrador</h4>
              </div>
              <p className="text-[11px] text-slate-400 leading-relaxed">
                Con la base de datos real, el administrador (Profe Luis) podrá registrar alumnos reales, congelar/anular categorías manualmente, publicar avisos masivos y auditar moras de forma permanente desde cualquier dispositivo con internet.
              </p>
            </div>
          </div>
          ...
          <div className="bg-amber-500/5 border border-amber-500/10 p-4 rounded-2xl text-center space-y-3">
            <p className="text-[11px] text-amber-500 font-bold uppercase tracking-wider">
              ⚠️ Siguiente Acción del Proyecto
            </p>
            <p className="text-xs text-slate-300 leading-relaxed max-w-xl mx-auto">
              Para dar de alta la base de datos segura y activar el control real de alumnos en internet, el administrador debe autorizar el inicio del montaje en servidores de producción.
            </p>
          </div>
        </div>
      </section>
