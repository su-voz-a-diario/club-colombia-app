...
export async function POST(request) {
  try {
    ...
    if (!resendApiKey || !fromEmail || !appUrl) {
      // Registrar error interno detallado sin información del usuario ni datos sensibles
    ...
    try {
      ...
      if (error) {
        // Registrar el error sin exponer datos sensibles del correo o del token
      ...
      // Capturar errores de Firebase (como usuario no encontrado / assert failed)
      // Evitamos logs de token o enlace y silenciamos para evitar enumeración.
      console.warn("Recuperación de contraseña abortada de forma segura.");
    }

    // Respuesta pública genérica idéntica para evitar la enumeración de cuentas
    return NextResponse.json({
      success: true,
    ...
    console.error("Error global en el handler de restablecimiento de contraseña.");
    return NextResponse.json(
      { success: false, error: "Ocurrió un error al intentar procesar la solicitud." },
      { status: 500 }
    );
  }
}
